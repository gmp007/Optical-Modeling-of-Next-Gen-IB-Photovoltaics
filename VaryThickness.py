...
class OMVaryThickness(TM):
    ...
    def PlotVaryJsc(self, target):
        ...
        axJsc.set_xlabel(xlabel, size=22)
        axJsc.set_ylabel(ylabel, size=22)
        axJsc.tick_params(labelsize=18, length = 10, width = 3)
        for axis in ['top','bottom','left','right']:
            axJsc.spines[axis].set_linewidth(3)
        axJsc.plot(self.t_range, [Jsc[target - 1] for Jsc in self.varyJsc],
                   '-o', linewidth=3, color='r', markersize=8)
        ...
        figJsc.tight_layout()
        figJsc.savefig("JscVsThick.pdf", transparent=False)
    ...
    def PlotVaryAbs(self, target, cbarlegend=False):
        ...
        axAbs.tick_params(labelsize=18, length = 10, width = 3)
        for axis in ['top','bottom','left','right']:
            axAbs.spines[axis].set_linewidth(3)
        ...
        figAbs.savefig("AbsVsThick.pdf", transparent=False)
