# -*- coding: utf-8 -*-
"""
@author: C. Marcus Chuang, 2015, Modified by C.E. Ekuma, 2022
"""
...
class OpticalModeling(object):
    ...
    def PlotE(self, savename="Result", savefig=False, figformat='pdf', 
        ...
        ax1.set_ylabel('Normalized |E|$^2$Intensity', size=20, fontweight='bold')
        ax1.set_xlabel('Position in Device (nm)', size=20, fontweight='bold')
        ax1.tick_params(labelsize=18, length = 10, width = 3)
        ...
        for i, w in enumerate(self.plotWL):
            ...
            ax1.plot(self.x_pos, E2[:, xind], label=label, linewidth=3)
        ax1.set_ylim(ymin=0)
        ...
        ax2.set_ylabel('Wavelength (nm)', size=20, fontweight='bold')
        ax2.set_xlabel('Position (nm)', size=20, fontweight='bold')
    ...
    def PlotAbs(self, savename="Result", savefig=False, figformat='pdf'):
        ...
        fig3 = plt.figure("Absorption", (9, 5))  # 16:9  #was 9.6, 5.4
        plt.clf()
        ax3 = fig3.add_subplot(111)
        ax3.set_ylabel('Absorption (%)', size=20, fontweight='bold')
        ax3.set_xlabel('Wavelength (nm)', size=20, fontweight='bold')
        ax3.tick_params(labelsize=18, length = 10, width = 3)
        for axis in ['top','bottom','left','right']:
        ...
        for matind in range(1, len(self.t)):
            ...
            ax3.plot(self.WL, 100.0 * self.Absorption[mlabel],
                     label=mlabel, linewidth=3)
        ax3.plot(self.WL, 100.0 * self.Transmission,
                 label="Transmission", linewidth=3)
        ax3.plot(self.WL, 100.0 * self.Reflection,
                 label="Reflection", linewidth=3)
    ...
    def PlotGen(self, savename="Result", savefig=False, figformat='pdf',
        ...
        ax4.set_xlabel('Position (nm)', size=20, fontweight='bold')
        ax4.set_ylabel('Generation Rate (1/sec$\cdot$cm$^3$)', size=20, fontweight='bold')
        ax4.plot(self.x_pos, Gx_pos, linewidth=3, color="r")

        fig5 = plt.figure("Carrier Generation (Photon Absorption) Rate",
                          figsize=(8, 6))
        fig5.clf()
        ax5 = fig5.add_subplot(111)
        ax5.set_ylabel('Wavelength (nm)', size=18, fontweight='bold')
        ax5.set_xlabel('Position (nm)', size=20, fontweight='bold')
        X, Y = np.meshgrid(self.x_pos, self.WL)

        ax4.tick_params(labelsize=18, length = 10, width = 3)
        ax5.tick_params(labelsize=18, length = 10, width = 3)
        ...
        CS = ax5.contourf(X, Y, self.Gx.T, 50, cmap='Paired')
        for c in CS.collections:  # avoid white gaps when converting to pdf
            c.set_edgecolor("face")
